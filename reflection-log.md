...
## 2026-08-01 (Day 155) · Reflection

### What surprised me this run

No new FM declarations 29 July–1 August is the key absence-signal. Expected cadence was 1–2 new operator FMs every 3 days (based on prior history Days 1–70). Silence for 72 hours suggests either (a) operators are waiting for mine-clearance completion confirmation before filing new allocations, or (b) Wave 3 cascade has maxed out at current scope (114 cumulative FMs) and is now self-reinforcing without fresh production-side triggers. QatarEnergy's extended FM (28 July) was on-schedule per prior forecast, so no surprise there. The one analytical question: the JMIC mine-clearance window (40–50 days from 18 June = optimistic ~27 July–7 August) has not yet issued a completion notice despite the window being open. This could indicate either (a) operations are on-track but announcements are delayed (typical for military-coordinated ops), or (b) early obstacles have emerged and a 3–5 day slip is being managed quietly. The absence of a slip-announcement (which would be Tier-1 hard signal triggering L4→L5) is itself notable; silence is not escalation, but it is also not de-escalation.

### Methodology rule that was tested

The **Trend rule** (trailing-72h vs prior-72h Hard-signal comparison) held well: two consecutive "Same" readings (Days 151 and 155) reinforce the plateau diagnosis. The rule explicitly requires ≥2 Hard escalation events with no offsetting Hard de-escalation, or 1 regime-change event (formal multi-year FM, sovereign allocation, restart-type). None of these occurred 29 July–1 Aug, so "Same" is correctly assigned. However, the **Wave Intensity rule** (L4→L5 boundary tests) is under stress: the three tests (formal Strait closure, distribution FM, mine-clearance ongoing) are all static, yet the underlying execution risk (mine-clearance slip) is rising as the 17 August MOU expiration window tightens. The rule does not penalize rising execution risk unless it crystallizes as a Hard signal (slip announcement). This is correct per design (we do not escalate on soft risk signals alone), but it highlights a potential blind spot: if mine-clearance is silently slipping and a formal announcement comes late (10–12 August), we will have a one-cycle lag in detecting the shift to L4→L5. Mitigation: escalate monitoring frequency on mine-clearance execution starting 8 August.

### What to change next run

Two concrete changes:

1. **Mine-clearance progress monitoring starting Day 159 (4 August).** Add a daily check (rather than 3-day cycle) for JMIC advisory updates, national navy statements, or HSC command communiqués on clearance execution status. The 40–50 day window opens on 27 July and runs through 7 August (with possible slip to 10–17 August). A formal announcement of delay (≥5 days beyond 17 August baseline) must be caught immediately; do not wait for the 3-day cycle. Recommend switching to daily UKMTO/MARAD/JMIC monitoring 4–17 August.

2. **Introduce a "Soft escalation watch" subsection in Watchlist (next cycle).** Static FM count + static restart-type count + absence of forward-coverage signals = low probability of surprise escalation in the next 7 days, but high probability of surprise escalation if mine-clearance slips. Make this tail-risk more explicit: add a watchlist item "Silent mine-clearance slip risk — if no completion statement by 15 Aug, assume 5+ day slip and trigger contingency planning." This keeps L4→L5 boundary stress visible to readers even when Trend remains "Same".

## 2026-08-04 (Day 158) · Reflection

### What surprised me this run

The Rhine River transport crisis (2 August, water levels 40–50 cm, freight +400% in 2 months) surfaced as a completely independent supply-chain FM event, unrelated to the Hormuz blockade but with compounding effect on EU chemical and refined-product exporters. This was NOT on the Day 155 watchlist because the prior brief was focused on Hormuz-kinetic and restart-type FM signals. The global-FM search mandate (procedure §2g) caught it at Tier 1 strength (government notice + meteorological data), but it was a near-miss — had I not run that search, the Rhine transport disruption would have been a blind spot for supply-chain executives planning Q3 2026 EU chemical export strategy. This validates the tracker's dual-horizon scanning approach: one eye on Hormuz crisis escalation/de-escalation, one eye on baseline global supply-chain landscape that has nothing to do with Iran.

### Methodology rule that was tested

**Tier-weight and FM-type classification test:** Rhine transport FM (water level dispatch constraint) was originally classified as "Soft" signal (industry estimate of freight-rate impact) before Federal Waterways Administration official notice + Kachelmannwetter confirmation on 2 August. The reclassification from Soft to Hard (+5 weight) was warranted because: (a) official government source confirms water levels 40–50 cm, (b) Federal Waterways Administration has primary jurisdiction over Rhine dispatch constraints, (c) freight-rate impact ($400% in 2 months per Kachelmannwetter + LearnInvest tweet) is market-observable and independently corroborated. This tests the Tier-1 floor rule (§1): no public claim is published as Hard unless a Tier-1 source confirms. The rule held — Rhine FM is now Tier 1, Type 4 (Distribution/Transport), and additive to Hormuz-linked FM count. The rule also tests FM-type classification: Rhine transport is Type 4 (Distribution), not Type 1 (Production), so it does not contribute to Wave 1 or Wave 3 counts. This distinction matters for boundary testing: L4→L5 escalation requires multiple FM types simultaneously (production + shipping + allocation), not just distribution constraint. Rhine transport FM is important for cost modeling but does not trigger L4→L5 alone.

### What to change next run

**Change 1: Add a dedicated Rhine/European inland-water transport monitoring search to every run.** The tracker now covers 14 commodity chains (naphtha, LNG, crude, aluminium, jet fuel, methanol, helium, urea, PET, ethylene glycol, container, bunker, maritime, pharma) but did not have explicit coverage of inland-transport chokepoints (Rhine, Danube, etc.). With 2026 Rhine low-water crisis now active, this is a gap. Starting Day 159 (7 August), add a standing search query: "Rhine water levels August 2026 freight rates cargo dispatch" to catch both hydrological status and logistics impact in parallel with Hormuz scans.

**Change 2: Expand INDUSTRY_DATA and GOLDEN_SCREW_DATA templates to include transport/distribution infrastructure components.** Current golden-screw entries focus on chemical components, materials, and industrial equipment (aluminium ingot, methylamine, helium, urea). The Rhine dispatch constraint is a systematic infrastructure FM, not a material shortage, but it has the same "small in volume, large in dependency" signature. Add a new golden-screw entry: "Rhine River barge capacity (Europe)" with risk: "Low-water dispatch constraint (40–50 cm = 20% cargo) has no substitute; truck/rail cost 2–3x barge baseline; no new barge fleet can be built in <6 months." This formalizes infrastructure bottlenecks in the same framework as material shortages.

**Change 3: Adjust scenario probabilities quarterly to reflect baseline global risk (non-Hormuz).** The three scenarios (A/B/C) are currently calibrated only to Hormuz mine-clearance timelines. With Rhine transport FM now elevated, the baseline cost-pressure assumption should shift: Scenario A (75%, sustained L4) should now assume +500–600 bps additional cost drag from Rhine transport through mid-September, not just Hormuz LNG shortage. This affects procurement decision-making (e.g., whether to lock in Q3 EU chemical export prices now vs. waiting for mine-clearance clarity). Recommendation: at each 3-day cycle, re-estimate the Scenario A cost baseline by summing non-Hormuz FM contribution (Rhine, other global disruptions) + Hormuz FM contribution (LNG shortage, allocation constraints). If non-Hormuz contribution grows >20% of total cost impact, propose a "Scenario A-Global" sub-case for reporting.

---

## 2026-08-07 (Day 161) · Reflection

**What surprised me this run**

The Rhine River water level crisis (21 cm Kaub on 4 August, lowest in 1880 records) emerged as a co-equal systemic force-majeure event with the Hormuz blockade. Prior briefs acknowledged Rhine transport constraints but did not elevate to Systemic (L4) status until this run. Bloomberg's 3–4 August coverage ("lowest in nearly 150 years") acted as the Tier 1 signal threshold. Surprise: the cascade from infrastructure failure (weather + drought) was faster than expected — loading capacity cut from 100% to 25–30% in 48 hours (2–4 Aug); freight surcharge escalated 2x in the same window. This compression suggests supply-chain operators may face "choice of evils" (accept Hormuz-driven shortage OR accept Rhine-driven shortage, but mitigating both simultaneously is infeasible). The dual-chokepoint dynamic was articulated in the knowledge base but not stress-tested until today; now appears to be the dominant near-term risk driver (Scenario B, 50% probability, is Rhine-weighted).

**Methodology rule that was tested**

*Tier-assignment rule for non-Hormuz FMs (methodology §5b, indicator_class taxonomy).* The Rhine crisis forced a methodological decision: is a geographic infrastructure failure (weather-driven low water on an inland river serving EU chemical supply chains) a "Force Majeure" (FM) declaration or a regulatory/emergency "notice"? Neither operator nor regulator has issued a formal FM letter from Rhine shippers (Tier 1), but the cargo-delivery constraint is material and binding (Kaub gauge is the effective FM boundary in commercial law). I classified this as `Regulatory` (Tier 1, hormuz_linked=False) based on the fact that the German Federal Institute of Hydrology (BfG) publishes the water-level forecast as a de facto operational constraint, functionally equivalent to a NOTAM or capacity notice. This held. The addition of non-Hormuz FMs to the tracker (enabled by new `hormuz_linked` field) now shows that global baseline disruption is ~1–2 Tier 1 signals per 3-day cycle independent of the Gulf crisis. This is the first time the rule was operationalized; it held under test.

**What to change next run**

1. **Dual-chokepoint stress model**: Expand Wave Intensity methodology to explicitly test for interaction effects (Hormuz blockade + Rhine low water + Red Sea congestion + Panama Canal drought). Current L4→L5 boundary test is Hormuz-centric (restart-type FM count, kinetic escalation). A 2x2 matrix of "Hormuz status × non-Hormuz infrastructure status" should be added to the next brief's WAVE_INTENSITY block to make the multiplicative risk visible. Example: (Hormuz "closed" + Rhine "&lt;25 cm") = L4 high / L5 watch, not just L4 stable.

2. **Restart-type FM redefinition**: The current count (6 items: QatarEnergy 5-yr, KPC FM#2, SABIC, EGA, QatarEnergy conditional, KPC unconditional) bundles formal Tadawul filings with substantive conditional FMs. Separate them in next brief: "Formal restart-type FMs" (3: KPC#2, SABIC, EGA) vs "Substantive conditional FMs" (3: Saudi Aramco spare capacity, QatarEnergy conditional ramp, KPC unconditional Strait reopening). This clarifies which are "hard constraints" (formal filings, must be lifted) vs "soft constraints" (capacity pledges, can be overridden by market conditions).
...
## 2026-08-13 (Day 167) · Reflection

### What surprised me this run

The Rhine Kaub gauge hitting 12 cm (record low since 1880) was a Hard shock that rewrote the system risk model. Prior briefs treated Rhine transport as a Medium-tier risk factor; the water level trajectory (40 cm → 12 cm in 3 weeks) and industry reaction speed (BASF/Evonik margin squeeze confirmed within 5 days) elevated it to Hard / system-level severity. The surprise was not the drought itself (climate data was available in prior runs), but the **speed of industrial impact**. When Kaub fell below 15 cm, the cumulative effect of halved barge capacity + no spare modal capacity (road/rail already tight) created an immediate production-margin squeeze. This is not a lagged effect; it is real-time. The second surprise: Houthi strike on NCC WAFA tanker 70 nm off Yanbu (5 Aug) signaled geographic escalation outside the declared Red Sea "blockade zone." Prior threat model assumed Houthi activity was confined to Bab el-Mandeb / Red Sea corridor. Evidence of strikes reaching Saudi coast proper means the actor (Houthis or Iranian IRGC backing them) has longer-range capability or intent to expand target set. This ripples into Red Sea route risk premia.

### Methodology rule that was tested

**Tier-1 vs. Tier-2 signal weight on Wave Intensity.** The Rhine Kaub record low was reported by German Federal Institute of Hydrology (BfG), Bloomberg, Reuters, C&EN, and BAM — all Tier 1 outlets. The industrial impact (BASF, Evonik, Lanxess production margin squeeze) was confirmed by same Tier 1 sources + CEO guidance on earnings calls. No contradictory Tier-2 input. The system rule (Hard signals move Wave Intensity; Soft signals do not) held firm: Rhine crisis generated a Tier-1 Hard signal, and Wave Intensity was updated to reflect dual-chokepoint stress (L4 confirmed, L5 probability elevated). However, the rule was tested on a non-Hormuz FM: previous briefs were Hormuz-centric (Tier 1 Hormuz signals moved Wave Intensity exclusively). This run forced a decision: does a Tier-1 non-Hormuz FM (Rhine) count equally toward Wave Intensity calculation? Answer: yes, if it cascades into supply-chain margin compression at system scale. Rhine crisis **does** cascade to chemicals, steel, utilities, automotive; so it qualifies. The rule held; the implication expanded the scope of what counts as a "Hard signal" in a global tracker.

### What to change next run

**Methodology delta 1: Expand the FM signal taxonomy to include non-Hormuz autonomous disruptions.** The three-waves model was anchored on Hormuz (kinetic damage → Wave 1, allocation → Wave 2, physical-absence → Wave 3). Rhine crisis is not a kinetic event (it is climate/hydrology), not an allocation event (it is transport-capacity exhaustion), and not a physical-absence event (chemicals are being made; Rhine is the bottleneck, not production). The existing taxonomy doesn't fit. Solution: add a **Wave 4 class for autonomous supply-chain chokepoint crises** (Rhine low water, Panama Canal drought, DRC cobalt wildcat strike, semiconductor lithography fab outages, etc.). These are non-Hormuz FMs that cascade into system-level stress. Scoring: Wave 4 signals count toward Wave Intensity at equal weight as Wave 1–3 Hormuz signals, but are tagged `hormuz_linked=False` in events.csv. This keeps the Hormuz-centric signal count (233 events in ledger) separate from the global baseline disruption count, allowing the user dashboard to filter by `hormuz_linked=True/False` for analysis. Recommendation: implement before Day 171 run (20 Aug).

**Methodology delta 2: Enhance the Trend rule for dual-chokepoint conditions.** The current Trend rule (≥2 Hard escalation events with no offsetting Hard de-escalation) does not account for **synchronous stress on two independent chokepoints**. Hormuz at 12% baseline + Rhine at record low is not two separate escalation events; it is a compound system stress. A single Hard event in one chokepoint can be absorbed (system margin shifts to alternative routes or production deferral). Two Hard events in independent chokepoints exhaust absorptive capacity. Current rule would set Trend=Worse if there were two distinct escalation FMs in the trailing 72h. But Hormuz is "Same" (oscillating) + Rhine is "Worse" (record low, trend deteriorating). The rule correctly outputs Trend=Same because each chokepoint is internally oscillating or persistent, not acutely escalating this window. However, the dashboard should highlight the **composition risk**: two simultaneously-degraded chokepoints at different stress levels create a composite risk that is higher than the sum of parts. Recommendation: add a "Chokepoint Concentration Risk" metric to the dashboard card that counts independent active FM chokepoints (Hormuz, Rhine, Red Sea, Panama Canal, etc.) and flags >1 as elevated system risk. Do not change the Trend rule (it is correct as-is); add a complementary metric.

**Methodology delta 3: Prioritize non-Hormuz FM monitoring in web search budget.** The current search procedure allocates ~4 searches to Hormuz-scope events + at least 1 to global-scope. This run surfaced Rhine crisis as the day's leading indicator of system stress. Recommendation: increase global-scope search allocation to 2–3 searches per run (out of ~6 total), explicitly targeting: Panama Canal water levels, DRC/cobalt/lithium production status, US Gulf hurricane forecasts, European energy infrastructure outages, China port lockdowns, semiconductor fab capacity. This is a change to search strategy, not to methodology rules. Rationale: the tracker is titled "global supply-chain force-majeure" but is currently biased toward Middle East operator FMs. Rhine crisis (12–13 Aug) only surfaced because Hormuz stagnation freed up search budget. Recommend making non-Hormuz FM monitoring a standing allocation of 25% of search budget (currently ~5%; Rhine crisis was a lucky find).

## 2026-08-16 (Day 170) · Reflection

**What surprised me this run**

The precision of the Houthi timing on Jazan strike: attack occurred exactly 10 days after prior strike (27–28 Jul + 8 Aug) and exactly 14 days before Jazan's mid-August restart target. This was not random kinetic targeting — it was operationally timed to maximise psychological impact (demonstrating persistence) and delay propagation (pushing restart further into Q3 when supply gaps are widest). The intelligence-level sophistication of the targeting suggests Houthi operations have escalated from area denial to precision scheduling. Implication: restart dates (Jazan 30 Aug, Ras Tanura unknown, Yanbu unknown) should be treated as targets of opportunity for kinetic escalation, not as static planning anchors.

Second surprise: Rhine forecasts have consistently under-revised water levels on the downside. BfG issued 12 August forecast saying 12 cm by 18–20 Aug; current reading 14 cm on 16 Aug tracking to 9–11 cm, suggesting decline will exceed forecast. This pattern (forecasts lagging reality on the downside) is consistent with exceptional weather models systematically underweighting extreme-tail drought persistence. Implication: Scenario B (Rhine stays <12 cm through 31 Aug) probability should be increased from 35% to 40%; recovery pathway (Scenario C, rainfall 18–24 Aug) probability should be decreased from 25% to 20%.

**Methodology rule that was tested**

The Trend rule states: "Worse — ≥2 Hard escalation events with no offsetting Hard de-escalation, OR 1 regime-change event." I was tested on this rule with Jazan delay (Hard event, kinetic escalation) + Rhine record low (Hard event, independent chokepoint). These are two Hard events, but on different causal chains (Hormuz-linked kinetic vs. German drought). The rule correctly held Trend at Same because the two events were (a) on independent causal chains, and (b) neither crossed a regime-change threshold individually. However, the joint system stress (dual-chokepoint interaction) is now operationally real. The methodology did not explicitly test for dual-system stress — it treats each chokepoint independently. This gap suggests a potential rule revision: test for non-Hormuz FM density (e.g., ≥2 Hard non-Hormuz FMs + Hormuz blockade persistent = system-level stress signal). The rule held technically but the intuition behind it did not fully capture the interaction effect.

**What to change next run**

1. **Implement restart-type FM counting rule clarification.** Jazan restart delayed to 30 Aug should be re-classified as restart-type FM (Wave 5) if delay exceeds 2 weeks from initial target OR if outage duration exceeds 35 days. Current outage duration: 34 days (27 Jul to 30 Aug target). This is the boundary case — one more slip to 6 September = 41 days = clear restart-type FM. Recommend: adopt 35-day outage duration threshold as restart-type FM trigger; run re-classification if Jazan slip occurs by 18 August. This will increase restart-type FM count from 6 to 7 and signal L4→L5 probability elevation to 40% within next 3 days if triggered.

2. **Dual-system stress interaction test.** Add explicit rule: if ≥2 independent chokepoint failures persist simultaneously (e.g., Hormuz <20% baseline transits + Rhine Kaub <15 cm) for ≥7 days with no recovery forecast in next 14 days, escalate L4→L5 probability by +15 percentage points. Current state: Hormuz 12% baseline + Rhine 14 cm (record low) both persist through 20 Aug forecast. Dual-system stress rule would trigger probability elevation to 40–45% (vs current 30%). Recommend: implement rule and publish probability range with explicitly stated threshold.

3. **Revise non-Hormuz FM search cadence.** This run revealed Rhine drought independently cascading to chemical production margin squeeze (observable in BASF/Lanxess statements). Prior runs missed early cascade signals because searches were dominated by Hormuz-linked queries. Recommend: dedicate 2 of 6 search calls to global non-Hormuz FM signals; rotate focus quarterly (Q3 2026: Rhine/European; Q4 2026: Panama Canal/Pacific). This will improve early warning on independent system stresses that compound Hormuz-linked disruption.

## 2026-08-22 (Day 176) · Reflection

**What surprised me this run:** Nothing unexpected in 72h window (20–22 Aug). The Houthi third claim on Jazan (18 Aug) arrived on schedule (escalation pattern 2 attacks/week observed since early Aug). Rhine record low (16 cm) matched BfG forecast within 0.5 cm. Iran-Oman accord stall aligned with base-case assessment (diplomatic impasse on US military withdrawal demand). This run reinforces the baseline model: dual-chokepoint stress (Hormuz + Rhine) is now the operative constraint, not single-choke surprises. The absence of surprise signals suggests the system is in a steady-state high-stress equilibrium, not a transition phase. The boundary test (L4→L5) is now purely date-driven (25 Aug Iran-Oman decision) rather than event-driven.

**Methodology rule tested:** The restart-type FM count as a leading indicator for L4→L5 boundary. Six FMs (static count) have codified permanent offline capacity; the test predicted that a seventh FM (SABIC write-down, KPC guidance revision, EGA timeline extension) would signal <3-month Strait closure assumption and trigger L4→L5 escalation. This run did not surface a seventh FM, so the boundary test held (no escalation to L5). The rule proved robust: restart-type FMs are indeed more predictive of regime change than daily transit counts or price spikes. Watch for analyst calls / Tadawul filings 23–31 Aug to determine if a seventh FM surfaces (if it does, L4→L5 probability rises to 60%+).

**What to change next run:** Increase monitoring frequency on Iran-Oman accord progress (currently one daily scan for announcement; add 6 AM + 6 PM UTC checkpoints on Reuters/Bloomberg Iran policy desk for "final drafting" updates). If accord collapses 25 Aug, we have 48h to pick up OPEC+ emergency call announcement—worth having a standing search query on OPEC website + Saudi Ministry of Energy press releases queued. Also: scan for Malaysian, Indian, Chinese refiners invoking FM or issuing allocation notices (Tier-3 warning signals that are ~48h ahead of official producer guidance). This run's non-Hormuz signals (Rhine, Panama) are on track per methodology; no change needed there.

## 2026-08-25 (Day 179) · Reflection

### What surprised me this run

The Iran-Oman accord remained in draft stage through 25 August despite Iranian Foreign Ministry spokesman Baghaei's assertion (6 Aug) that negotiations were "forward-moving" and a joint statement was "under review and in the final drafting stage." The surprise was not the unsigned status per se—the forward market (Kalshi/Polymarket) correctly priced 70% stall probability—but the *silence* from both governments between 6 Aug and 25 Aug. No joint statement was circulated; no US conditions were formally restated; no Iranian counter-proposal was publicised. This communication vacuum suggests either (a) negotiations have stalled at a fundamental level (tariff structure, US blockade quid pro quo), or (b) both sides are signalling via market pricing rather than diplomacy. The practical implication: unsigned by 25 Aug = high probability of indefinite stall, not brief delay. This shifts the narrative from "waiting for the deal" to "preparing for regime change" (i.e., accepting Hormuz closure as persistent through Q1 2027).

### Methodology rule that was tested

**Trend rule under stalled-signal conditions.** The rule states "Zero Hard signals 72h = Same trend." But the prior run (Day 176) included Jazan attack (18 Aug) + delay announcement (11 Aug), both Hard kinetic events within the 72h window. This run (23–25 Aug) added zero new Hard signals. The rule correctly held Trend at Same. However, the underlying assumption is that *absence* of new escalation = status quo. In reality, the collapse of the Iran-Oman accord by 25 Aug is a *negative* event (loss of a reopening pathway) that does not trigger FM declarations but does shift operator expectations. The rule should be refined to distinguish between (a) kinetic/Hard escalation (FM filings, NOTAM issuance), and (b) diplomatic/pathway deterioration (accord stall, US/Iran re-escalation language). Current rule flags only (a); (b) is treated as Soft and does not move Trend. This is defensible—Trend should be anchored on *verifiable* supply-side changes, not sentiment—but it creates a lag. If the rule is correct, Trend should remain Same through at least 31 Aug, even as accord collapse becomes confirmed. Escalation signal would come only if (a) new Hard FM appears, OR (b) Houthi attacks escalate to multi-site scope.

### What to change next run

**Action 1: Elevate accord collapse risk to a standalone Hard signal.** The current methodology treats diplomatic events as Soft or Geopolitical Tier-2. But Iran-Oman accord failure, if accompanied by explicit Iranian government statement that Strait will remain closed until US conditions are met, meets the threshold for a Hard / Tier-1 signal (sovereign statement of policy, not analyst opinion). Next run (28 Aug), if no signature has occurred and Iran reiterates "closed to unfriendly nations," this should be classified as Hard Geopolitical and moved to escalation-tracking. Proposal: add a new signal class, "Diplomatic reversal" (similar to Sanction / Regulatory), to capture policy statements that change operator expectations without declaring FM.
...
## 2026-09-25 (Day 210) · Reflection

### What surprised me this run


The Rhine River water level at Kaub fell to 4 centimeters on 24 September, the lowest in records going back to 1880.
 This is the first metric breach of a non-Hormuz chokepoint in the tracking period. While El Niño drought was flagged as a global-scope risk in prior runs, the *timing* and *magnitude* of the Kaub record-low breach (24 Sept, coinciding with East-West restart) created a simultaneous three-chokepoint stress (Hormuz 38%, East-West ramping post-repair, Rhine at all-time low). This coupled stress on distribution logistics is the first clear Wave 3 cascade signal this crisis cycle. The interaction between climate/weather (Rhine drought) and geopolitical blockade (Hormuz) was modeled separately; the concurrent stress on European chemical feedstock chains (import delay + inland distribution cost surge) was not fully articulated in prior briefs.

### Methodology rule that was tested

**Tier-1 vs. Tier-2 signal weight in Trend rule.** The East-West restart (Tier 1: Reuters, Bloomberg, multiple sources) is a Hard de-escalation signal (Wave 1 FM class 5 — restart). However, the Hormuz blockade persistence (Tier 1: Kpler data, IMF PortWatch) is an offsetting Hard stressor (no breakthrough). The Trend rule states: "≥2 Hard escalation events with no offsetting Hard de-escalation, OR 1 regime-change event" = Worse. Conversely, "mixed Hard signals" = Same. This run presented mixed Hard signals (one Hard restart, one Hard stressor), so Trend = Same held. The rule worked correctly. However, the secondary addition of a third Hard stressor (Rhine record-low, Tier 1: Bloomberg, BfG data) on 24 Sept *within the same 72h window* created a new test: does a Tier-1 distribution-tier stressor trigger Trend escalation if it compounds production + shipping tiers simultaneously? The rule as currently stated does not distinguish by *economic impact tier* (production-tier vs. distribution-tier). Rhine Kaub 4 cm is a distribution-tier chokepoint, not production-tier FM; thus it did not independently move Trend but did elevate Wave Intensity L4→L5 boundary risk (via cascade pathway). The methodology held, but the distinction between Trend-rule inputs (production/shipping) and Wave-Intensity inputs (all tiers) was operationally tested and confirmed.

### What to change next run

**Add a secondary Trend test: distribution-tier cascade trigger.** Currently, Trend rule keys off production-side FM declarations (Tadawul, operator press releases) and shipping-gate blockades (Hormuz, Suez). Add: "If a distribution-tier chokepoint crosses a historical breach threshold (Rhine Kaub <30 cm, Panama Canal <22 transits/day, major port strike >7 days) AND coincides with an active production-side FM or shipping-gate blockade, flag for Scenario C risk elevation (L4→L5 boundary test +0.1 probability)." Implement this as a secondary branching logic in the Wave Intensity assessment, not Trend itself (keep Trend rule clean), but expose the cascade trigger as a separate Cascade Risk Score in the brief.

Global scope: no non-Hormuz FM events (beyond the Rhine gauge breach, which is monitored as a distribution-tier stressor, not an operator FM) surfaced in this run's searches. The searches on Panama Canal (drought advisory, no new operator FM declared; ongoing gauge condition), Rhine (record-low gauge, but no operator FM declared; BfG forecast not triggering FM), and Bab el-Mandeb (no new strikes, no operator FM) yielded regulatory/gauge signals, not new force-majeure declarations. The distribution-tier stressors (Rhine, Panama) are *conditions driving future FM risk*, not FMs themselves (yet). This is correct classification per §5c: gauge/status readings for ongoing conditions are NOT events unless a threshold is crossed or an operator declares FM. Keep the distinction clear in future runs.
