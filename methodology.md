...
**Methodology delta 2026-06-25 (Day 118).**

**Section 2 (Trend rule), rule adjustment:** "When Hard geopolitical statement is contradicted by Hard physical/market data within 24 hours, prioritize physical data; weight state statement as Soft (procedural noise) until enforcement is attempted kinetically." **Reason:** Day 118 Trend calculation: Iran's 21 Jun re-closure announcement (Hard) vs. 25 AIS transits 22 Jun (Hard) created signal contradiction. Prior methodology treated both as equal weight. Corrected approach: falsified state claims (unconfirmed by ground truth within 24h) revert to Soft tier pending enforcement. This aligns with May 2026 observed pattern (multiple Iranian closures announced, none enforced kinetically; market learned to discount rhetoric).
...
## 2026-09-14 (Day 199) · Methodology Delta — §5c Ledger hygiene *(added after the Day-198 ledger audit)*

**Trigger.** Audit of `events.csv` on 14 Sep 2026 found: (a) 19 rows that were the same incident re-submitted under a different spelling, a later date or a second source (Houthi strike on tanker *Amzan* 24 Aug ×3; Iran–Oman Strait framework 8 Aug ×3; PGSA toll regime 18 May ×3; QatarEnergy CEO ramp halt 9 Jul ×3; Rhine gauge 4 Aug ×2; Jazan third strike logged 18 Aug and again 22 Aug; Stolt Magnesium 14 Jul ×2; Houthi NCC WAFA 5 Aug ×2; QatarEnergy mid-June extension 12 May ×2; IRGC Strait redefinition 15 May ×2) plus four Rhine gauge status readings logged as separate events, one of which (25 Aug, "Kaub 6 cm") was contradicted by the source record (Ports Europe: Kaub 75 cm on 25 Aug; the 6 cm minimum was 14 Aug); (b) five real events lost to column-shift rejections (Panama Canal 25 Aug, Covestro 12 Aug, Jazan 18 Aug, Iran MFA 25 Aug, US Military 2 Sep); (c) the dashboard feed capped at 220 rows, so tier tiles showed T1=176 against T1=215 in the ledger and no event before 7 March reached the page; (d) global (non-Hormuz) coverage collapsed to Rhine/Panama only (10 of 260 rows), missing Covestro/BASF FMs, Typhoon Saudel port closures, the ver.di and FNV port strikes, the Panama slot/draft advisories, CMA CGM's Manaus low-water surcharge, Valero Port Arthur after TS Edouard and the Grasberg restart guidance.

**Rule §5c.1 — Near-duplicate test (script-enforced).** A candidate row is a duplicate of an existing row when: same date AND entity-token match (subset or Jaccard ≥ 0.5 after stripping corporate/role filler and plural *s*) AND ≥ 1 shared chain token; or |Δdate| ≤ 4 days with the same entity/chain match AND summary token-Jaccard ≥ 0.35. Duplicates are not counted; the attempt is logged in `count-log.md` under *Near-duplicates not counted*. Distinct chains on the same date (QatarEnergy helium / LNG / urea, 28 Feb) remain separate rows.

**Rule §5c.2 — Status readings are not events.** A gauge, transit-count or price reading for an *ongoing* condition (Rhine/Kaub, Gatún, Mississippi, Hormuz daily transits, OSP) is logged only at a threshold crossing: first breach of the operating reference (GlW 77 cm at Kaub; ACP slot or draft change; record value), the recovery above it, or an operator action caused by it (FM declared/lifted, surcharge introduced/changed). Daily readings between crossings go in the brief text, not the ledger.

**Rule §5c.3 — Quarantine, never drop.** A row that fails validation is written to `events-quarantine.csv` with the reason and re-presented to the model for up to 21 days / 20 rows. It leaves quarantine when an accepted row with the same date and entity lands. The parser first re-aligns column-shifted rows by anchoring on the tail (`…, is_eu_direct, source, summary, indicator_class, tier, hormuz_linked`), so a collapsed `,,` middle triplet or an unquoted comma in the summary no longer discards the event.

**Rule §5c.4 — Full-ledger feed.** `RECENT_EVENTS_DATA` carries every row of `events.csv` (no cap). Every dashboard tile that counts events is computed from that array, so page = file by construction.

**Rule §5c.5 — Global-scope quota.** Each run receives a script-computed directive: a rotating non-Hormuz theme (canals & rivers → ports & labour → mining & metals → chemicals & pharma → energy & power → semiconductors/autos → agri & food) and the count of non-Hormuz rows dated in the last 14 days. If that count is ≤ 1, at least two of the run's searches must be global-scope. A run that surfaces nothing global must say so in REFLECTION with the queries used.
